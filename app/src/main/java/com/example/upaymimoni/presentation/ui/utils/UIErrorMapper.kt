...
object UpdateUserErrorMapper {
    fun toFieldError(error: UpdateUserError): FieldError {
        return when (error) {
            ...
            UpdateUserError.NetworkUnavailable -> FieldError(
                FieldType.NONE,
                "Network error. Please try again."
            )

            UpdateUserError.NotFound -> FieldError(
                FieldType.NONE,
                "An error occurred. Please try again."
            )

            UpdateUserError.PermissionDenied -> FieldError(
                FieldType.NONE,
            ...
            is UpdateUserError.Unknown -> FieldError(
                FieldType.NONE,
...
object AuthErrorMapper {
    fun toFieldError(error: AuthError): FieldError {
        return when (error) {
            AuthError.InvalidCredentials -> FieldError(
                FieldType.INPUT,
                "Incorrect Email or Password."
            )

            AuthError.InvalidUser -> FieldError(FieldType.INPUT, "Incorrect Email or Password.`")
            AuthError.NetworkFailure -> FieldError(
                FieldType.NONE,
                "Network error, please try again."
            )

            AuthError.TooManyLogins -> FieldError(
                FieldType.NONE,
                "Too many attempts, try again later."
            )

            AuthError.EmailInUse -> FieldError(
                FieldType.EMAIL,
                "An account with that email is already in use."
            )

            is AuthError.WeakPassword -> FieldError(
                FieldType.PASSWORD,
                "Password too weak: ${error.reason}"
            )
            ...
            is AuthError.Internal -> FieldError(
                FieldType.NONE,
                "Something went wrong on our end, please try again."
            )

            is AuthError.Unknown -> FieldError(
                FieldType.NONE,
    ...
    fun toForgotPasswordFieldError(error: AuthError): FieldError {
        return when (error) {
            AuthError.InvalidUser -> FieldError(FieldType.INPUT, "No account found with that email.")
            AuthError.NetworkFailure -> FieldError(
                FieldType.NONE,
                "Network error, please try again."
            )
